let taomlar = [
    ...
    const div = document.querySelector('.taomlar-container');
function createCard(taom) {
    return `
        <div class="taom-card">
            <img src="${taom.rasm}" alt="">
            <h3>${taom.nom}</h3>
            <p>${taom.matn}</p>
            <p class="price">${taom.narx} so'm</p>
        </div>
    `;
}
...
function app(type) {
    ...
    div.innerHTML = '';
    const filteredTaomlar = taomlar.filter(taom => taom.tur === type);
    filteredTaomlar.forEach(taom => {
        div.innerHTML += createCard(taom);
    });
}
